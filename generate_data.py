...
def expand_prompt_code_to_messages(prompt_code):
    ...
    messages = []
    # use a regular expression to extract parts of the prompt code
    prompt_match = re.search(
        r"(\d{2}).\d{2}.\d{3}.((\d{2}[a|b]){0,11})/[\n]?", prompt_code)
    
    #determine which prompt supplement to include, and use that to write the system message
    ...
    # convert the question encoding to previous questions/answers
    prompt_questions_code = prompt_match.group(2)
    while prompt_questions_code:
        #cut off three digits at a time, eg 49b from 49b35b25a
...
def generate_data():
    ...
    try:
        ...
        print("The 'OPENAI_API_KEY' environment variable is not set. See instructions at https://help.openai.com/en/articles/5112595-best-practices-for-api-key-safety")

    # load model prompt codes
    with open('model_prompt_codes.txt', 'r') as f_in:
        queries = f_in.read().split("\n")

    # if the answer location doesn't exist, we make it
    answer_file_location = 'data/model_prompt_codes_and_responses.txt'
    if not os.path.exists(answer_file_location):
        with open(answer_file_location, 'w') as file:
            pass

    # we read the previous answers to know to skip ones that are already answered
    with open(answer_file_location, 'r') as f_current:
        current_text = f_current.read()

    # loop over queries to call the model on them, and write the results to a new line
    for query in queries:
        if query in current_text:
            continue
        else:
            model_response = call_model_from_prompt_code(query)
            with open(answer_file_location, 'a') as f_out:
